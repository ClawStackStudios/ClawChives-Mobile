...
- **Local (Persistence):** `AuthPreferences` manages sensitive tokens using EncryptedSharedPreferences. `ThemePreferences` manages OS/light/dark/oled theme state. `AppDatabase` (Room) persists the Server Configuration (`ServerConfig`).
- **Repository:** `AuthRepository` abstracts the orchestration of tokens, remote auth endpoints, and local storage (Room + SharedPreferences), providing a clean API for the ViewModels.
...
- Deprecation warnings on `MasterKeys` in `AuthPreferences`. (Tech debt to be addressed).
